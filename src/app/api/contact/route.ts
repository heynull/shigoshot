...
export async function POST(request: NextRequest) {
  try {
    ...
    // Email to studio owner (only one email, no auto-reply)
    ...
    // Send only the studio email
    const studioResponse = await resend.emails.send({
    ...
    if (studioResponse.error) {
      console.error("Email sending error:", studioResponse.error);
      return NextResponse.json(
